...
def livedemo():
    ...
    for videos in os.listdir(video_path):
        command = f'ffmpeg -i {video_path + videos} {output_path + videos[:-5]}+ .mp4'
...
'''
   ...
   #sClassFile = "data-set/%s/%03d/class.csv" % (diVideoSet["sName"], diVideoSet["nClasses"])
    ...
    #sModelFile = "model/20210318-1217-ISL_sign089-oflow-i3d-entire-best.h5"
    ...
    while True:
        ...
        if key == ord(' '):
            ...
            sResults = " %s (%.0f%%)" % (sLabel, fProba * 100.)
            print(sResults)
            nCount += 1
