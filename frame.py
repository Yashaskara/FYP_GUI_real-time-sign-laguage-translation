...
def images_normalize(arFrames: np.array, nFrames: int, nHeight: int, nWidth: int, bRescale: bool = True) -> np.array(
    float):
...
#         return int(check_output(["mediainfo", '--Inform=Video;%Duration%', sVideoPath], shell=True))/1000.0
...
def videosDir2framesDir(sVideoDir: str, sFrameDir: str, nFramesNorm: int = None,
    ...
    dfVideos = pd.DataFrame(sorted(glob.glob(sVideoDir + "/*/*/*.mp4")), columns=["sVideoPath"])
    ...
    for sVideoPath in dfVideos.sVideoPath:
        ...
        if len(li_sVideoPath) < 3: raise ValueError(
            "Video path should have min 4 components: {}".format(str(li_sVideoPath)))
        ...
        # length and fps
        fVideoSec = get_length(sVideoPath)
        ...
        print("Video %5d | %5.1f sec | %d frames | %4.1f fps | saved %s in %s" % (
        nCounter, fVideoSec, nFrames, fFPS, str(arFrames.shape), sTargetDir))
        nCounter += 1
...
def unittest(sVideoDir, nSamples=100):
    ...
    liVideos = glob.glob(sVideoDir + "/*/*.avi") + glob.glob(sVideoDir + "/*/*.mp4")
    ...
    nCount = i + 1
    print("%d samples: Average video duration %.1f sec, fps %.1f" % (
    nSamples, fVideoSec_sum / nCount, nFrames_sum / fVideoSec_sum))
...
if __name__ == '__main__':
    ...
    unittest("data-set/ISL_sign/89/train/", 100)