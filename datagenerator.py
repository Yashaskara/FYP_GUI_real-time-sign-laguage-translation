...
class FramesGenerator(keras.utils.Sequence):
    ...
    def __init__(self, sPath: str, \
        ...
        # print('NSAMPLES:', self.nSamples) #NSAMPLES has total number of frames
        ...
        seLabels = self.dfVideos.sFrameDir.apply(lambda s: s.split('/')[-2])  # .split('\\')[-2])
        self.dfVideos.loc[:, "sLabel"] = seLabels
        print(self.dfVideos.sFrameDir[1])
        # print(self.dfVideos)
        # print('SELABLES:', seLabels)
        # extract unique classes from all detected labels
        self.liClasses = sorted(list(self.dfVideos.sLabel.unique()))
        # print('CLASSES:', len(self.liClasses))
